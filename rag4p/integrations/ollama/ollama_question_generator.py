...
class OllamaQuestionGenerator(QuestionGenerator):
    ...
    def generate_question(self, context: str) -> str:
        ...
        """
        prompt = f"""You are a content writer reading a text and writing questions that are answered in that text. 
Use the context as provided and nothing else to come up with one question. 
The question should be a question that a person that does not know a lot about the context could ask. Do not use names in your question or exact dates. 
Do not use the exact words in the context. Each question must be one sentence only end always end with a '?' character. 
The context is provided after 'context:'. The result should only contain the generated question, nothing else.
context: 
{context}
generated question:
"""
